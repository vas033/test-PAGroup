...
interface IMyList {
  ...
  longText?: boolean;
  longTextPart?: string;
  clLongTextPart?: string;
  cl?: string;
  clLi?: string;
}

export function List({ img = false, item, cl, clLi, longText, longTextPart, clLongTextPart }: IMyList) {
  let listItem;

  if (longText && longTextPart) {
    if (clLongTextPart) {
      listItem = <p className={clLi}>
        <span>
          {item}
        </span>
        <span className={clLongTextPart}>
          {longTextPart}
        </span>
      </p>
    } else {
